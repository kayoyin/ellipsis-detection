...
class Tagger(abc.ABC):
    ...
    def ellipsis(self, src, ref, align, verbs, nouns, ellipsis_sent, qe_score, qe_threshold):
        ...
        for i, tok in enumerate(ref):
            if (
                ...
                if i not in align.values() and tok.lemma_ in verbs and ellipsis_sent and qe_score < qe_threshold:
                    tags[i] = True
            ...
            if (
                ...
                if i not in align.values() and tok.lemma_ in nouns and ellipsis_sent and qe_score < qe_threshold:
                    tags[i] = True
...
def main():
    ...
    parser.add_argument("--qe-file", type=str, default=None, help="file with qe scores")
    parser.add_argument("--qe-threshold", type=float, default=0.1)
    ...
    if args.qe_file is not None:
        with open(args.qe_file, "r", encoding="utf-8") as file:
            qe_scores = [float(line.strip()) for line in file if len(line.strip()) > 0] 
    else:
        qe_scores = [-np.inf] * len(srcs)
    alignments = list(
        map(
            lambda x: dict(
    ...
    with open(args.output, "w", encoding="utf-8") as output_file:
        for (
            ...
            ellipsis_tags, verbs, nouns = tagger.ellipsis(
                cur_src_doc, cur_tgt_doc, align, verbs, nouns, ellipsis_sent, qe_score, args.qe_threshold
            )
